...
import { Edit, Trash2, FileStack, MoreHorizontal } from 'lucide-react';
import {
    Table,
    TableBody,
    TableCell,
...
export const TemplateList: React.FC<TemplateListProps> = ({ onEdit, onDelete }) => {
    const templates = useQuery(api.templates.queries.list, { paginationOpts: { numItems: 20, cursor: null } });
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'APPROVED': return 'default'; // varying based on theme, usually primary
            case 'REJECTED': return 'destructive';
            case 'PAUSED': return 'secondary';
            case 'DRAFT': return 'outline';
            case 'PENDING': return 'secondary'; // or yellow if we had warning variant
